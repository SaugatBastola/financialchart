...
(function($) {
            ...
            function() {
                ...
                this.construct = function(settings) {
                    return this.each(function() {
                        ...
                        this.config = {};
                        config = $.extend(this.config, $.sorttable.defaults, settings);
                        $this = $(this);
                        $.data(this, "sorttable", config);
    ...
    $.fn.extend({ sorttable: $.sorttable.construct });
    var ts = $.sorttable;
    ...
    ts.addParser({
        ...
        is: function(s, table) {
            var c = table.config;
            return $.sorttable.isDigit(s, c);
        },
        format: function(s) { return $.sorttable.formatFloat(s); },
        type: "numeric"
    });
    ts.addParser({ id: "numbers", is: function(s) { return false; }, format: function(s) { return s.replace("$", "").replace(/,/g, ""); }, type: "numeric" });
    ts.addParser({ id: "currency", is: function(s) { return/^[£$€?.]/.test(s); }, format: function(s) { return $.sorttable.formatFloat(s.replace(new RegExp(/[£$€]/g), "")); }, type: "numeric" });
    ts.addParser({
        ...
        format: function(s) {
            ...
            return $.sorttable.formatFloat(r);
        },
        type: "numeric"
    });
    ...
    ts.addParser({ id: "isoDate", is: function(s) { return/^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/.test(s); }, format: function(s) { return $.sorttable.formatFloat((s != "") ? new Date(s.replace(new RegExp(/-/g), "/")).getTime() : "0"); }, type: "numeric" });
    ts.addParser({ id: "percent", is: function(s) { return/\%$/.test($.trim(s)); }, format: function(s) { return $.sorttable.formatFloat(s.replace(new RegExp(/%/g), "")); }, type: "numeric" });
    ts.addParser({ id: "usLongDate", is: function(s) { return s.match(new RegExp(/^[A-Za-z]{3,10}\.? [0-9]{1,2}, ([0-9]{4}|'?[0-9]{2}) (([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(AM|PM)))$/)); }, format: function(s) { return $.sorttable.formatFloat(new Date(s).getTime()); }, type: "numeric" });
    ts.addParser({
        ...
        format: function(s, table) {
            ...
            return $.sorttable.formatFloat(new Date(s).getTime());
        },
        type: "numeric"
    });
    ts.addParser({ id: "time", is: function(s) { return/^(([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(am|pm)))$/.test(s); }, format: function(s) { return $.sorttable.formatFloat(new Date("2000/01/01 " + s).getTime()); }, type: "numeric" });
    ts.addParser({
    ...
    ts.addWidget({
        ...
        format: function(table) {
            ...
            if (table.config.debug) {
                $.sorttable.benchmark("Applying Zebra widget", time);
            }
        }
    });
})(jQuery);